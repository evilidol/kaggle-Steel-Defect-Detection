...
import torch
from lovász_loss import lovasz_hinge, binary_xloss, lovasz_hinge_steel, xloss
################### DICE ########################
...
class CE_SOFT(nn.Module):
    def __init__(self):
        super(CE_SOFT, self).__init__()
...
class Lovasz_Hinge(nn.Module):
    ...
    def forward(self, logit, truth):
        ...
        return lovasz_hinge_steel(logit, truth, per_image=self.per_image)
...
class BCE_Lovasz(nn.Module):
    ...
    def forward(self, logit, truth):
        ...
        bce = xloss(logit, truth)
        # lovasz = lovasz_hinge(logit, truth, per_image=self.per_image)
        lovasz = lovasz_hinge_steel(logit, truth, per_image=self.per_image)
        return bce + lovasz